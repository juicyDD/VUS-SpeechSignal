...
filePath='D:\TinHieuHuanLuyen-44k\';
files = {'30FTN', '42FQT', '44MTT', '45MDV','01MDA','02FVA','03MAB','06FTB'};
...
for i=1:number_of_frames
    if(subVUS(i+2)==1)
        ...
        count = count + 1;
        frame=x(((i-1)*floor(samples_per_frame/3)+1):((i-1)*floor(samples_per_frame/3)+samples_per_frame));
        %frame=x(((i-1)*floor(samples_per_frame.)+1):((i-1)*floor(samples_per_frame)+samples_per_frame));
         %foreach 8Hz, there's ONE fft point
        ft = fft(frame.*blackman(samples_per_frame),floor(Fs/8)); %ft is the fourier transform of this frame
        ft =log(abs(ft)+1); 
        %ft =abs(ft);
...
%ft=fftshift(ft);
plot(1:8:floor(Fs/8)*8,ft(1:floor(Fs/8)));
title('Fast Fourier Transform(FFT) of an arbitrary windowed frame');
