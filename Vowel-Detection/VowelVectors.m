function [VA,VE,VI,VO,VU]=VowelVectors()
   A=[];E=[];I=[];O=[];U=[];
for imlooping = 1:21
...
[c,tc]=v_melcepst(mysignal,Fs,'M',39, floor(3*log(Fs)),floor(0.03*Fs),floor(0.03*Fs/2),0,0.5);
%c=mean(c);
switch imlooping2
    case 1
        A=[A,c'];
...
% A :5,E:7, I:5,O:4, U:5
[VA,G,J,GG]=v_kmeans(A,1);
[VE,G,J,GG]=v_kmeans(E,1);
[VI,G,J,GG]=v_kmeans(I,1);
[VO,G,J,GG]=v_kmeans(O,1);
[VU,G,J,GG]=v_kmeans(U,1);
%VA=mean(A);VE=mean(E);VI=mean(I);VO=mean(O);VU=mean(U);
%------------------------------
%---------------------------------
%-----------------KIEMTHU-------------
%-----------------KIEMTHU----------------
